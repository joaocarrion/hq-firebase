...
export interface ListItem {
    id: string; // the unique id of the list item
    listUnit: number; // 6 for unitary, 3 for kg
    product: string; // id of the product (must be type product)
    quantity: number; // if unitary, 1 to buy 1, if kg, 500 for 0.5kg
}

export interface PurchasedProduct {
  product: string; // product id
  quantity: number; // number of units or weight in grams
  unitPrice: number; // in cents
  totalPrice: number; // in cents
  totalDiscount?: number; // in cents, optional
}
